...
description: 瞭解如何使用自訂參數實作Target的Visual Experience Composer(VEC)、將它與裝置配對，以及建立活動。 本課是「在Mobile iOS Swift應用程式中實作Experience cloud」教學課程的一部分。
...
* 完成「設定啟動」區 [段中的課程](launch-create-a-property.md) 。
...
1. 在Xcode中，開啟 `BookingViewController.swift` 檔案。 此檔案由主螢幕使用。
...
1. 在函式 `viewDidLoad()` 中，在行後加上 `super.viewDidLoad()` 下列程式碼。 此范常式式碼顯示如何將參數、描述檔參數、產品（或實體）參數和訂單參數新增至TargetVEC請求。 此範例使用靜態值，而實際應用程式中您想使用動態變數來填入值。 當然，您只想填入與檢視相關的參數：
   ...
   ![新增參數至TargetVEC請求](images/ios/swift/mobile-targetvec-addParameters.png)
...
1. 點擊 `Enter` 可跳至Target請求和貼文內文。 找出您剛新增至請求的自訂參數：
   ...
   ![驗證TargetVEC請求的參數](images/ios/swift/mobile-targetvec-verifyParams.png)
...
1. 展開部 `URL Types` 分
1. 請注意， **[!UICONTROL Identifier]** is set to `com.adobetarget.BusBookingSwift`. 您可以使用此識別碼，或視需要變更。
1. 請注意， **[!UICONTROL URL配置]** 為 `BusBookingSwift`。 您可以使用此方案，或視需要變更。
1. 請確定「 **[!UICONTROL 角色]** 」已設為「編 **[!UICONTROL 輯器」]**

   ![註冊URL配置](images/ios/swift/mobile-targetvec-registerScheme.png)
...
1. 如下圖 `ACPCore.collectLaunchInfo(["adb_deeplink": url.absoluteString])` 所示將行 `AppDelegate:application:openURL` 添加到區
   ![更新AppDelegate檔案](images/ios/swift/mobile-targetvec-appDelegate.png)

### 驗證深層連結

現在，當安裝您應用程式的使用者在模擬器中開啟類似URL( `BusBookingSwift://com.adobetarget.BusBookingSwift` 或您定義的任何配置)的URL時，就會開啟您的應用程式。
...
1. 重建應用程式
1. 在模擬器中，開啟Safari
1. 在位址 `BusBookingSwift://com.adobetarget.BusBookingSwift` 列中輸入URL（或您定義的任何配置）。 如果您遇到任何困難，請參閱下方的提示部分。
1. 您應會收到「在BusBookingSwift中開啟本頁」的提示。 如果您遇到任何困難，請參閱下方的提示部分。
1. 按一下 `Open`
1. 這應該會開啟「巴士預訂」應用程式
   ...
   ![驗證深層連結](images/ios/swift/mobile-targetvec-verifyDeepLink.png)
...
1. 在「輸入URL配置」欄位中 **[!UICONTROL 輸入您剛定義的URL配置]** ，例如 `BusBookingSwift://com.adobetarget.BusBookingSwift`
1. 按一下「 **[!UICONTROL 建立深層連結」]**

   ![輸入您的URL配置並建立深層連結](images/ios/swift/mobile-targetvec-enterURLScheme.png)
...
1. 按一下連結上的任何位置，即可自動將連結複製到剪貼簿
   ...
   ![複製URL](images/ios/swift/mobile-targetvec-copyURL.png)
